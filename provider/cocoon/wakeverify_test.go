...
func TestResolveWakeSourceVerifiedLocalHit(t *testing.T) {
	rt := &fakeRuntime{snapshots: map[string]*vm.Snapshot{
		"vk-ns-demo-0-505043": {Name: "vk-ns-demo-0-505043", ID: "SNAP-1"},
	}}
	p := newTestProvider(t)
	p.Runtime = rt
	...
	if src.localName != "vk-ns-demo-0-505043" || src.snapshot == nil || src.snapshot.ID != "SNAP-1" {
...
func TestResolveWakeSourceStaleLocalDiscardsAndPulls(t *testing.T) {
	rt := &fakeRuntime{snapshots: map[string]*vm.Snapshot{
		"vk-ns-demo-0-505043": {Name: "vk-ns-demo-0-505043", ID: "SNAP-OLD"},
	}}
	p := newTestProvider(t)
	p.Runtime = rt
	p.Registry = newWakeVerifyRegistry(t, "SNAP-NEW")

	_, err := p.resolveWakeSource(t.Context(), "ns", "vk-ns-demo-0-505043")
	if err == nil || !strings.Contains(err.Error(), "no puller configured") {
		t.Fatalf("err = %v, want fall-through to pull path", err)
	}
	if !slices.Contains(rt.snapshotRemoveCalls, "vk-ns-demo-0-505043") ||
		!slices.Contains(rt.snapshotRemoveCalls, forkSnapshotName("vk-ns-demo-0-505043")) {
...
func TestWakeStagesFromPeerAndClonesFromDir(t *testing.T) {
	...
	if src.snapshot == nil || src.snapshot.ID != "SNAP-REMOTE" || src.snapshot.Name != "vk-ns-demo-0-505043" {
	...
	spec := meta.VMSpec{VMName: "vk-ns-demo-0-505043", Network: "cocoon-dhcp"}
...
func newPeerWakeFixture(t *testing.T, snapshotID string) (*Provider, *fakeRuntime) {
	...
	sourceRT := &fakeRuntime{snapshots: map[string]*vm.Snapshot{
		"vk-ns-demo-0-505043": {Name: "vk-ns-demo-0-505043", ID: snapshotID},
	}}
