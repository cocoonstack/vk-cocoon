...
func TestCreatePodEvidenceFailClosedOnRegistryError(t *testing.T) {
	...
	pod := newPodWithSpec(meta.VMSpec{VMName: "vk-ns-demo-0-505043", Image: "snapshot-repo:latest", Mode: "clone"})
	err := p.CreatePod(t.Context(), pod)
...
func TestCreatePodEvidenceForkFromConflict(t *testing.T) {
	...
	pod := newPodWithSpec(meta.VMSpec{VMName: vmName, ForkFrom: "vk-ns-main-0-f89f7a", Mode: "run"})
	err := p.CreatePod(t.Context(), pod)
...
func TestCreatePodPreMarkedRestoreKeepsForkFrom(t *testing.T) {
	const vmName = "vk-ns-demo-1-d239dc"
	rt := &fakeRuntime{snapshots: map[string]*vm.Snapshot{vmName: {Name: vmName, ID: "SNAP-1"}}}
	p := newTestProvider(t)
	p.Runtime = rt
	p.Registry = newWakeVerifyRegistryWithImage(t, "SNAP-1", "reg.example/agent:v1")

	pod := newPodWithSpec(meta.VMSpec{VMName: vmName, Image: "reg.example/agent:v1", ForkFrom: "vk-ns-demo-0-505043", Mode: "clone"})
	pod.Annotations[meta.AnnotationRestoreFromHibernate] = "true"
	if err := p.CreatePod(t.Context(), pod); err != nil {
		t.Fatalf("create: %v", err)
	}
