...
func TestCreatePodCloneMode(t *testing.T) {
	...
	if rt.cloned.To != "vk-ns-demo-0-505043" {
...
func TestCreatePodForkFromLocalVMSkipsSnapshotBaseImage(t *testing.T) {
	rt := &fakeRuntime{inspectVM: &vm.VM{ID: "source-vm-id", Name: "vk-ns-demo-0-505043"}}
	p := newTestProvider(t)
	p.Runtime = rt

	pod := newPodWithSpec(meta.VMSpec{
		VMName:   "vk-ns-demo-1-d239dc",
		Image:    "snapshot-repo:latest",
		Mode:     "clone",
		ForkFrom: "vk-ns-demo-0-505043",
	})
	if err := p.CreatePod(t.Context(), pod); err != nil {
		t.Fatalf("create: %v", err)
	}
	...
	if rt.savedSnapshot.name != "fork-vk-ns-demo-0-505043" || rt.savedSnapshot.vmID != "source-vm-id" {
		t.Fatalf("SnapshotSave = (%q, %q), want (fork-vk-ns-demo-0-505043, source-vm-id)", rt.savedSnapshot.name, rt.savedSnapshot.vmID)
	}
	if rt.cloned.From != "fork-vk-ns-demo-0-505043" {
		t.Fatalf("clone source = %q, want fork-vk-ns-demo-0-505043", rt.cloned.From)
	}
	if len(rt.ensuredImages) != 0 {
...
func TestEnsureForkSnapshotDedupsConcurrentSaves(t *testing.T) {
	...
	p.Runtime = rt
	p.mu.Lock()
	p.vmsByName["vk-ns-demo-0-505043"] = &vm.VM{ID: "src", Name: "vk-ns-demo-0-505043"}
	p.mu.Unlock()
	...
	for i := range n {
		wg.Go(func() {
			names[i], errs[i] = p.ensureForkSnapshot(t.Context(), "vk-ns-demo-0-505043")
		})
	}
	<-entered
	close(release)
	...
	want := forkSnapshotName("vk-ns-demo-0-505043")
	for i := range n {
		if errs[i] != nil {
...
func TestCreatePodInvalidatesForkSnapshot(t *testing.T) {
	tests := []struct {
		...
		{
			name: "run mode",
			rt:   &fakeRuntime{runVM: &vm.VM{ID: "vmid-main", Name: "vk-ns-demo-0-505043"}},
			spec: meta.VMSpec{VMName: "vk-ns-demo-0-505043", Image: "registry.example/cocoon/ubuntu:24.04", Mode: "run", OS: "linux"},
		},
		{
			name: "clone mode",
			rt:   &fakeRuntime{snapshots: map[string]*vm.Snapshot{"snapshot-repo": {Name: "snapshot-repo"}}},
			spec: meta.VMSpec{VMName: "vk-ns-demo-0-505043", Image: "snapshot-repo:latest", Mode: "clone"},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(tt.rt.snapshotRemoveCalls) != 1 || tt.rt.snapshotRemoveCalls[0] != "fork-vk-ns-demo-0-505043" {
				t.Fatalf("SnapshotRemoveIfExists calls = %v, want [fork-vk-ns-demo-0-505043]", tt.rt.snapshotRemoveCalls)
			}
		})
	}
}
...
func TestCreatePodDoesNotReclaimSupersededIncarnationAfterBringUp(t *testing.T) {
	...
	p.Runtime = &fakeRuntime{
		runVM: &vm.VM{ID: "vmid-a", Name: "vk-ns-demo-0-505043"},
		runHook: func() {
			p.trackPod(podB, nil)
		},
	}
...
func TestApplyRuntimeRejectsRecreatedPodAtAPIFence(t *testing.T) {
	...
	p.Clientset = client
	p.Runtime = &fakeRuntime{inspectVM: &vm.VM{ID: "vmid-a", Name: "vk-ns-demo-0-505043", State: vm.StateRunning}}
	p.trackPod(podA, nil)

	if p.applyRuntime(t.Context(), podA, &vm.VM{ID: "vmid-a", Name: "vk-ns-demo-0-505043"}) {
...
func TestUpdatePodRoutesARecreatedPodThroughCreate(t *testing.T) {
	...
	p.Runtime = &fakeRuntime{inspectVM: &vm.VM{ID: "vmid-a", Name: "vk-ns-demo-0-505043", State: vm.StateRunning}}
	p.trackPod(podA, &vm.VM{ID: "vmid-a", Name: "vk-ns-demo-0-505043"})
...
func TestApplyRuntimeKeepsTheBindingWhenThePodIsGone(t *testing.T) {
	...
	if !p.applyRuntime(t.Context(), pod, &vm.VM{ID: "vmid-a", Name: "vk-ns-demo-0-505043"}) {
...
func TestDetachIncarnationKeepsSuccessorBinding(t *testing.T) {
	p := newTestProvider(t)
	podB := newPodWithSpec(meta.VMSpec{VMName: "vk-ns-demo-0-505043", Mode: "run"})
	podB.UID = "b"
	p.trackPod(podB, &vm.VM{ID: "vmid-b", Name: "vk-ns-demo-0-505043"})

	p.detachIncarnation(meta.PodKey("ns", "demo-0"), "a", &vm.VM{ID: "vmid-a", Name: "vk-ns-demo-0-505043"})

	if got := p.vmByName("vk-ns-demo-0-505043"); got == nil || got.ID != "vmid-b" {
...
func TestCreatePodForkFromReusesExistingSnapshot(t *testing.T) {
	rt := &fakeRuntime{
		inspectVM: &vm.VM{ID: "source-vm-id", Name: "vk-ns-demo-0-505043"},
		snapshots: map[string]*vm.Snapshot{
			"fork-vk-ns-demo-0-505043": {Name: "fork-vk-ns-demo-0-505043"},
		},
	}
	p := newTestProvider(t)
	p.Runtime = rt
	...
	if rt.cloned == nil || rt.cloned.From != "fork-vk-ns-demo-0-505043" {
		t.Fatalf("clone source = %q, want fork-vk-ns-demo-0-505043", rt.cloned.From)
	}
}
...
func TestCreatePodCloneModePassesCPUPolicy(t *testing.T) {
	...
	pod := newPodWithSpec(meta.VMSpec{
		VMName: "vk-ns-clone-policy-21c9da",
		Image:  "golden",
	})
	pod.Spec.Containers = []corev1.Container{{
		Resources: corev1.ResourceRequirements{
...
func TestPodCPUPolicy(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			pod := newPodWithSpec(meta.VMSpec{VMName: "vk-ns-policy-538d5a"})
...
func TestCreatePodRunErrorPropagates(t *testing.T) {
	...
	if got := p.vmByName("vk-ns-run-5fa295"); got != nil {
		t.Errorf("failed CreatePod should not track a VM, got %#v", got)
	}
}
...
func TestDeletePodRemovesAndForgetsVM(t *testing.T) {
	...
	p.trackPod(pod, &vm.VM{ID: "vmid-del", Name: "vk-ns-demo-0-505043"})
	...
	wantSnapRemovals := slices.Sorted(slices.Values([]string{"vk-ns-demo-0-505043", forkSnapshotName("vk-ns-demo-0-505043")}))
...
func TestCreatePodUnmanagedAdoptsExistingVM(t *testing.T) {
	...
	meta.VMSpec{VMName: "vk-ns-static-55bf1d", Mode: "static", Managed: false}.Apply(pod)
...
func TestStartupReconcileAdoptsAnUnmanagedPodFromItsAnnotations(t *testing.T) {
	for _, local := range []bool{false, true} {
		...
		rt := &fakeRuntime{}
		if local {
			rt.listVMs = []vm.VM{{ID: "extern-vm-1", Name: "vk-ns-cs-db-bc21fc", IP: "10.0.0.9"}}
		}
		p := newTestProvider(t)
...
func TestStartupReconcileAdoptsAnnotatedPods(t *testing.T) {
	...
	rt := &fakeRuntime{
		listVMs: []vm.VM{{ID: "adopted-vmid", Name: "vk-ns-demo-0-505043", IP: "10.0.0.42"}},
	}
	p := newTestProvider(t)
	p.NodeName = "cocoon-pool"
	p.Runtime = rt
	...
	if got := p.vmByName("vk-ns-demo-0-505043"); got == nil || got.ID != "adopted-vmid" {
...
func TestStartupReconcileOrphanAlertIndexesByName(t *testing.T) {
	rt := &fakeRuntime{
		listVMs: []vm.VM{{ID: "live-vmid", Name: "vk-ns-demo-0-505043", IP: "10.0.0.42"}},
	}
	p := newTestProvider(t)
	p.NodeName = "cocoon-pool"
	p.Runtime = rt
	...
	if got := p.vmByName("vk-ns-demo-0-505043"); got == nil || got.ID != "live-vmid" {
...
func TestStartupReconcileTracksHibernatedPodWithoutVM(t *testing.T) {
	pod := newPodWithSpec(meta.VMSpec{VMName: "vk-ns-demo-0-505043", Mode: "clone", Managed: true})
	pod.Spec.NodeName = "cocoon-pool"
	meta.HibernateState(true).Apply(pod)
...
func TestHandleVMGoneSkippedWhenPodDeleting(t *testing.T) {
	rt := &fakeRuntime{inspectVM: &vm.VM{ID: "vmid-d", Name: "vk-ns-demo-0-505043", State: "stopped"}}
	p := newTestProvider(t)
	p.Runtime = rt
	p.Clientset = fake.NewSimpleClientset()

	pod := newPodWithSpec(meta.VMSpec{VMName: "vk-ns-demo-0-505043", Mode: "run"})
	p.trackPod(pod, &vm.VM{ID: "vmid-d", Name: "vk-ns-demo-0-505043"})
	p.mu.Lock()
...
func TestHandleVMGoneInlineRetryRecoversFromTransient(t *testing.T) {
	rt := &fakeRuntime{
		inspectSeq: []fakeInspectStep{
			...
			{vm: &vm.VM{ID: "vmid-r", Name: "vk-ns-demo-0-505043", State: vm.StateRunning}},
		},
	}
	p := newTestProvider(t)
	p.Runtime = rt
	...
	pod := newPodWithSpec(meta.VMSpec{VMName: "vk-ns-demo-0-505043", Mode: "run"})
	p.trackPod(pod, &vm.VM{ID: "vmid-r", Name: "vk-ns-demo-0-505043"})

	p.handleVMGone(t.Context(), &vm.VM{ID: "vmid-r", Name: "vk-ns-demo-0-505043"})
...
func TestHandleVMGoneDeferredTimeoutKeepsLeaseWhenRemovalFails(t *testing.T) {
	...
	pod := newPodWithSpec(meta.VMSpec{VMName: "vk-ns-demo-0-505043", Mode: "run"})
	p.Clientset = fake.NewSimpleClientset(pod)
	p.trackPod(pod, &vm.VM{ID: "vmid-safe", Name: "vk-ns-demo-0-505043", MAC: "aa:bb:cc:dd:ee:03"})
...
func TestHandleVMGoneDeferredRecheckDedups(t *testing.T) {
	...
	p.Clientset = fake.NewSimpleClientset()
	pod := newPodWithSpec(meta.VMSpec{VMName: "vk-ns-demo-0-505043", Mode: "run"})
	p.trackPod(pod, &vm.VM{ID: "vmid-x", Name: "vk-ns-demo-0-505043"})
...
func TestHandleVMGoneCooldownKeepsRecreatedIncarnation(t *testing.T) {
	...
	client := fake.NewSimpleClientset(podB)
	p.Clientset = client
	base := &fakeRuntime{inspectVM: &vm.VM{ID: "vmid-a", Name: "vk-ns-demo-0-505043", State: "stopped"}}
	...
	p.handleVMGone(t.Context(), &vm.VM{ID: "vmid-a", Name: "vk-ns-demo-0-505043"})
...
func TestHandleVMGoneRemovalFencesRecreatedIncarnation(t *testing.T) {
	podA := newPodWithSpec(meta.VMSpec{VMName: "vk-ns-demo-0-505043", Image: "registry.example/cocoon/ubuntu:24.04", Mode: "run"})
	podA.UID = "a"
	podB := podA.DeepCopy()
	podB.UID = "b"
	...
	p.Clientset = client
	base := &fakeRuntime{
		inspectVM: &vm.VM{ID: "vmid-a", Name: "vk-ns-demo-0-505043", State: "stopped"},
		runVM:     &vm.VM{ID: "vmid-b", Name: "vk-ns-demo-0-505043", State: vm.StateRunning},
	}
...
func TestCreatePodCreatesAfreshWhenTheIndexedVMIsGone(t *testing.T) {
	pod := newPodWithSpec(meta.VMSpec{VMName: "vk-ns-demo-0-505043", Image: "registry.example/cocoon/ubuntu:24.04", Mode: "run"})
	rt := &fakeRuntime{runVM: &vm.VM{ID: "vmid-new", Name: "vk-ns-demo-0-505043", State: vm.StateRunning}}
	p := newTestProvider(t)
	p.Runtime = rt
	p.Clientset = fake.NewSimpleClientset(pod)
	p.mu.Lock()
	p.indexOrphanByNameLocked(&vm.VM{ID: "vmid-gone", Name: "vk-ns-demo-0-505043"})
	p.mu.Unlock()
	...
	if got := p.vmByName("vk-ns-demo-0-505043"); got == nil || got.ID != "vmid-new" {
...
func TestCreatePodRetriesWhenTheIndexedVMInspectIsInconclusive(t *testing.T) {
	pod := newPodWithSpec(meta.VMSpec{VMName: "vk-ns-demo-0-505043", Image: "registry.example/cocoon/ubuntu:24.04", Mode: "run"})
	rt := &fakeRuntime{inspectErr: errors.New("cocoon: transient"), runVM: &vm.VM{ID: "vmid-new", Name: "vk-ns-demo-0-505043"}}
	p := newTestProvider(t)
	p.Runtime = rt
	p.Clientset = fake.NewSimpleClientset(pod)
	p.inlineInspectBaseDelay = time.Millisecond
	p.mu.Lock()
	p.indexOrphanByNameLocked(&vm.VM{ID: "vmid-old", Name: "vk-ns-demo-0-505043"})
	p.mu.Unlock()
	...
	if got := p.vmByName("vk-ns-demo-0-505043"); got == nil || got.ID != "vmid-old" {
...
func TestCreatePodAdoptsTheIndexedVMWhileItLives(t *testing.T) {
	pod := newPodWithSpec(meta.VMSpec{VMName: "vk-ns-demo-0-505043", Image: "registry.example/cocoon/ubuntu:24.04", Mode: "run"})
	live := &vm.VM{ID: "vmid-live", Name: "vk-ns-demo-0-505043", State: vm.StateRunning}
	rt := &fakeRuntime{inspectVM: live, runVM: &vm.VM{ID: "vmid-new", Name: "vk-ns-demo-0-505043"}}
	p := newTestProvider(t)
	p.Runtime = rt
	p.Clientset = fake.NewSimpleClientset(pod)
	p.mu.Lock()
...
func TestUntrackKeepsAnOrphanIndexedUnderTheSameName(t *testing.T) {
	...
	podB.UID = "b"
	p := newTestProvider(t)
	vmA := &vm.VM{ID: "vmid-a", Name: "vk-ns-demo-0-505043"}
	vmB := &vm.VM{ID: "vmid-b", Name: "vk-ns-demo-0-505043"}
	p.trackPod(podA, vmA)
...
func TestEvictGoneIncarnationSkipsWhenSuccessorOwnsTheKey(t *testing.T) {
	...
	key := meta.PodKey("ns", "demo-0")
	p.evictGoneIncarnation(t.Context(), key, podA, &vm.VM{ID: "vmid-a", Name: "vk-ns-demo-0-505043"}, "VMGone", "vm no longer exists")
...
func TestGetPodStatusRefreshesIPFromLease(t *testing.T) {
	...
	pod := newPodWithSpec(meta.VMSpec{VMName: "vk-ns-demo-0-505043", Mode: "run"})
	p.trackPod(pod, &vm.VM{ID: "vmid", Name: "vk-ns-demo-0-505043", MAC: "aa:bb:cc:dd:ee:ff"})
...
func TestCreatePodAdoptPublishesStatusBeforeReady(t *testing.T) {
	...
	p.Clientset = client
	p.Runtime = &fakeRuntime{inspectVM: &vm.VM{ID: "vmid-adopt", Name: "vk-ns-demo-0-505043", IP: "192.0.2.10", State: vm.StateRunning}}
	p.trackPod(pod, &vm.VM{ID: "vmid-adopt", Name: "vk-ns-demo-0-505043", IP: "192.0.2.10"})
...
func TestCreatePodStaticToolboxPublishesReadyWithoutPostClone(t *testing.T) {
	...
	}, "vk-ns-cs-db-bc21fc", cocoonv1.SnapshotPolicyAlways).Apply(pod)
	meta.VMRuntime{VMID: "extern-vm-1", IP: "10.0.0.9"}.Apply(pod)
...
func TestCreateAndDeleteStaticMacosToolboxSkipTheMacosLifecycle(t *testing.T) {
	...
	meta.FromToolboxSpec(cocoonv1.ToolboxSpec{
		...
		Mode: cocoonv1.ToolboxModeStatic,
		OS:   cocoonv1.OSMacos,
	}, "vk-ns-cs-mac-69a309", cocoonv1.SnapshotPolicyAlways).Apply(pod)
...
func TestHandleVMGoneLeavesAnUnmanagedVMAlone(t *testing.T) {
	rt := &fakeRuntime{inspectVM: &vm.VM{ID: "extern-vm-1", Name: "vk-ns-cs-db-bc21fc", State: "stopped"}}
	p := newTestProvider(t)
	p.Runtime = rt
	pod := &corev1.Pod{Name: "cs-db", Namespace: "ns"}
	meta.VMSpec{VMName: "vk-ns-cs-db-bc21fc", Mode: "static", Managed: false}.Apply(pod)
	...
	p.handleVMGone(t.Context(), &vm.VM{ID: "extern-vm-1", Name: "vk-ns-cs-db-bc21fc"})
...
func TestEnsureForkSnapshotSurvivesProviderShutdown(t *testing.T) {
	...
	go func() {
		_, err := p.ensureForkSnapshot(t.Context(), "vk-ns-main-0-f89f7a")
		done <- err
	}()
	<-entered
	p.lifecycleStop()
...
func newWedgedForkFixture(t *testing.T) (*Provider, *fakeRuntime, chan struct{}, chan struct{}) {
	...
	p.Runtime = rt
	p.mu.Lock()
	p.vmsByName["vk-ns-main-0-f89f7a"] = &vm.VM{ID: "vmid-main", Name: "vk-ns-main-0-f89f7a", State: vm.StateRunning}
	p.mu.Unlock()
