...
func TestClassifyNICRecoveryStopsAtBudgetWithoutRetryingEvidence(t *testing.T) {
	...
	evidence, ok := p.classifyNICRecovery(pod, "vk-ns-demo-0-505043")
...
func TestResumeReadyWaitPublishesReadyForAnUnmanagedPod(t *testing.T) {
	...
	meta.VMSpec{VMName: "vk-ns-cs-db-bc21fc", Mode: "static", Managed: false, OS: string(cocoonv1.OSWindows)}.Apply(pod)
	meta.VMRuntime{VMID: "extern-vm-1", IP: "10.0.0.9"}.Apply(pod)
	meta.HibernateState(true).Apply(pod)
	pod.Annotations[meta.AnnotationLifecycleState] = string(meta.LifecycleStateCreating)
	v := &vm.VM{ID: "extern-vm-1", Name: "vk-ns-cs-db-bc21fc", IP: "10.0.0.9", State: vm.StateRunning}
	p.trackPod(pod, v)
