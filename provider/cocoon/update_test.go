...
func TestHibernateDropsNICOnCHWindows(t *testing.T) {
	...
	v := &vm.VM{ID: "vmid-1", Name: "vk-ns-demo-0-505043"}
...
func TestHibernateSkipsNICDropOnNonCHWindows(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			v := &vm.VM{ID: "vmid-x", Name: "vk-ns-demo-0-505043"}
...
func TestHibernateClearsVMIDBeforeRemove(t *testing.T) {
	...
	v := &vm.VM{ID: "vmid-pre", Name: "vk-ns-demo-0-505043"}
	if err := p.hibernate(t.Context(), pod, meta.ParseVMSpec(pod), v); err != nil {
		t.Fatalf("hibernate: %v", err)
	}
...
func TestHibernateRestoresVMIDOnRemoveFailure(t *testing.T) {
	...
	v := &vm.VM{ID: "vmid-live", Name: "vk-ns-demo-0-505043", IP: "10.0.0.7"}
...
func TestHibernateRemoveFailureKeepsAStaticNICsFreshIP(t *testing.T) {
	fresh := &vm.VM{ID: "vmid-1", Name: "vk-ns-demo-0-505043", IP: "10.0.0.8", MAC: "02:00:00:00:00:02", NetworkConfigs: []*vm.NetworkConfig{{Network: &vm.NetworkInfo{IP: "10.0.0.8"}}}}
	...
	v := &vm.VM{ID: "vmid-1", Name: "vk-ns-demo-0-505043", IP: "10.0.0.7", MAC: "02:00:00:00:00:01", NetworkConfigs: []*vm.NetworkConfig{{Network: &vm.NetworkInfo{IP: "10.0.0.7"}}}}
	p.trackPod(pod, v)
...
func TestHibernateRemoveFailureRepublishesTheLiveNICNotTheReleasedIP(t *testing.T) {
	rt := &fakeRuntime{removeErr: errors.New("remove boom"), inspectVM: &vm.VM{ID: "vmid-1", Name: "vk-ns-demo-0-505043", MAC: "02:00:00:00:00:02"}}
	p := newTestProvider(t)
	p.Runtime = rt
	pod := newPodWithSpec(meta.VMSpec{
		VMName:  "vk-ns-demo-0-505043",
	...
	meta.VMRuntime{VMID: "vmid-1", IP: "10.0.0.7"}.Apply(pod)
	v := &vm.VM{ID: "vmid-1", Name: "vk-ns-demo-0-505043", IP: "10.0.0.7", MAC: "02:00:00:00:00:01"}
	p.trackPod(pod, v)
...
func TestHibernateFailsOnNICDropGenericErr(t *testing.T) {
	...
	v := &vm.VM{ID: "vmid-3", Name: "vk-ns-demo-0-505043"}
...
func TestResolveWakeSourceUsesLocalSnapshot(t *testing.T) {
	rt := &fakeRuntime{snapshots: map[string]*vm.Snapshot{"vk-ns-demo-0-505043": {Name: "vk-ns-demo-0-505043"}}}
	p := newTestProvider(t)
	p.Runtime = rt

	src, err := p.resolveWakeSource(t.Context(), "ns", "vk-ns-demo-0-505043")
	if err != nil {
		t.Fatalf("resolveWakeSource: %v", err)
	}
	if src.localName != "vk-ns-demo-0-505043" || src.dir != "" {
		t.Errorf("source = %+v, want local snapshot vk-ns-demo-0-505043", src)
	}
	if src.snapshot == nil || src.snapshot.Name != "vk-ns-demo-0-505043" {
...
func TestResolveWakeSourceErrorsWhenLocalMissingAndNoPuller(t *testing.T) {
	...
	if _, err := p.resolveWakeSource(t.Context(), "ns", "vk-ns-demo-0-505043"); err == nil {
...
func TestHibernateReleaseFailureDoesNotBlock(t *testing.T) {
	...
	if err := p.hibernate(t.Context(), pod, meta.ParseVMSpec(pod), &vm.VM{ID: "vmid-1", Name: "vk-ns-demo-0-505043"}); err != nil {
...
func TestHibernateRenewSurvivesCancelledContext(t *testing.T) {
	...
	if err := p.hibernate(ctx, pod, meta.ParseVMSpec(pod), &vm.VM{ID: "vmid-1", Name: "vk-ns-demo-0-505043"}); err == nil {
		t.Fatal("hibernate should fail when the NIC drop fails")
	}
	got := execArgvs(rt)
	if len(got) != 2 || got[1] != "cmd /c ipconfig /renew" {
...
func TestResolveVMIPRefusesWriteToSwappedVM(t *testing.T) {
	...
	p.trackPod(pod, &vm.VM{ID: "vmid-successor", Name: "vk-ns-demo-0-505043"})

	stale := &vm.VM{ID: "vmid-old", Name: "vk-ns-demo-0-505043", MAC: "aa:bb:cc:dd:ee:ff"}
...
func TestUpdatePodReadsPodBeforeTracking(t *testing.T) {
	p := newTestProvider(t)
	base := newPodWithSpec(meta.VMSpec{VMName: "vk-ns-demo-0-505043", Backend: string(cocoonv1.BackendCloudHypervisor)})
	key := meta.PodKey(base.Namespace, base.Name)
	p.trackPod(base.DeepCopy(), &vm.VM{ID: "vmid-1", Name: "vk-ns-demo-0-505043"})
...
func TestHibernateStopsTheReadinessProbe(t *testing.T) {
	...
	v := &vm.VM{ID: "vmid-1", Name: "vk-ns-demo-0-505043", IP: "10.0.0.5", State: vm.StateRunning}
	p.trackPod(pod, v)
...
func newDropNICWakeFixture(t *testing.T, budget, interval time.Duration) (*Provider, *corev1.Pod, *vm.VM) {
	...
	v := &vm.VM{ID: "vmid-wake", Name: "vk-ns-demo-0-505043"}
	p.trackPod(pod, v)
