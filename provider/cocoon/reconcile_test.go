...
func TestStartupReconcileSkeletonCollectedNotAdopted(t *testing.T) {
	...
	rt := &fakeRuntime{
		listVMs: []vm.VM{{ID: "skel-vmid", Name: "vk-ns-demo-0-505043", State: vm.StateCreating}},
	}
	p := newTestProvider(t)
	p.NodeName = "cocoon-pool"
	p.Runtime = rt
...
func TestStartupReconcileBusyCreateIndexedAfterCommit(t *testing.T) {
	...
	rt := &fakeRuntime{
		...
		inspectSeq: []fakeInspectStep{
			{vm: &vm.VM{ID: "inflight-vmid", Name: "vk-ns-demo-0-505043", State: vm.StateCreating}},
			{vm: &vm.VM{ID: "inflight-vmid", Name: "vk-ns-demo-0-505043", State: vm.StateCreated}},
			{vm: &vm.VM{ID: "inflight-vmid", Name: "vk-ns-demo-0-505043", State: vm.StateCreated}},
		},
...
func TestStartupReconcileBusyCreateDeadOnArrivalGetsOrphanPolicy(t *testing.T) {
	...
	rt := &fakeRuntime{
		listVMs:             []vm.VM{{ID: "inflight-vmid", Name: "vk-ns-demo-0-505043", State: vm.StateCreating}},
		staleCreateOutcomes: map[string]vm.StaleCreateOutcome{"inflight-vmid": vm.StaleCreateBusy},
		inspectVM:           &vm.VM{ID: "inflight-vmid", Name: "vk-ns-demo-0-505043", State: "error"},
...
func TestWatchBusyCreateReclaimsAfterOwnerDies(t *testing.T) {
	rt := &fakeRuntime{
		...
		inspectVM: &vm.VM{ID: "inflight-vmid", Name: "vk-ns-demo-0-505043", State: vm.StateCreating},
	}
	p := newTestProvider(t)
	p.NodeName = "cocoon-pool"
	p.Runtime = rt
...
func TestStartupReconcileSkeletonNotCreatingReinspectsAndAdopts(t *testing.T) {
	...
	rt := &fakeRuntime{
		...
		inspectVM:           &vm.VM{ID: "won-vmid", Name: "vk-ns-demo-0-505043", State: vm.StateRunning, IP: "10.0.0.7"},
	}
	p := newTestProvider(t)
	p.NodeName = "cocoon-pool"
	p.Runtime = rt
	...
	got := p.vmByName("vk-ns-demo-0-505043")
...
func TestStartupReconcileNotCreatingCreatedKeepsWatching(t *testing.T) {
	...
	rt := &fakeRuntime{
		...
		staleCreateOutcomes: map[string]vm.StaleCreateOutcome{"won-vmid": vm.StaleCreateNotCreating},
		inspectSeq: []fakeInspectStep{
			{vm: &vm.VM{ID: "won-vmid", Name: "vk-ns-demo-0-505043", State: vm.StateCreated}},
			{vm: &vm.VM{ID: "won-vmid", Name: "vk-ns-demo-0-505043", State: vm.StateCreated}},
		},
		inspectVM: &vm.VM{ID: "won-vmid", Name: "vk-ns-demo-0-505043", State: vm.StateRunning, IP: "10.0.0.7"},
	}
...
func TestStartupReconcileNotCreatingDeadRecordGetsOrphanPolicy(t *testing.T) {
	rt := &fakeRuntime{
		listVMs:             []vm.VM{{ID: "won-vmid", Name: "vk-ns-demo-0-505043", State: vm.StateCreating}},
		staleCreateOutcomes: map[string]vm.StaleCreateOutcome{"won-vmid": vm.StaleCreateNotCreating},
		inspectVM:           &vm.VM{ID: "won-vmid", Name: "vk-ns-demo-0-505043", State: "stopped"},
	}
	p := newTestProvider(t)
	p.NodeName = "cocoon-pool"
	p.Runtime = rt
...
func TestStartupReconcileVerbErrorWatchesWithoutAdopting(t *testing.T) {
	rt := &fakeRuntime{
		...
		inspectVM:      &vm.VM{ID: "skel-vmid", Name: "vk-ns-demo-0-505043", State: vm.StateCreating},
	}
	p := newTestProvider(t)
	p.NodeName = "cocoon-pool"
	p.Runtime = rt
